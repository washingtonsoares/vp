...
public class KNN {
    ...
    public Pair[][] execute(DistanceMatrix dmat) {
        ...
        if (this.nrNeighbors > dmat.getElementCount() - 1) {
            throw new IllegalArgumentException("Number of neighbors bigger than the number of "
    ...
    public Pair[] execute(AbstractMatrix matrix, AbstractDissimilarity diss, int index) {
        long start = System.currentTimeMillis();
        Pair[] neighbors = null;
    ...
    public Pair[] execute(AbstractMatrix matrix, AbstractDissimilarity diss, AbstractVector refvector) {
        long start = System.currentTimeMillis();
        Pair[] neighbors = null;
