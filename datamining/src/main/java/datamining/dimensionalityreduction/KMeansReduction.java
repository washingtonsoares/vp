 ...
 */
public class KMeansReduction extends AbstractDimensionalityReduction {
    ...
    @Override
    protected float[][] execute(AbstractMatrix matrix, AbstractDissimilarity diss) {
    ...
    private ArrayList<ArrayList<Integer>> doClustering(float[][] points) {
        ArrayList<ArrayList<Integer>> clusters = new ArrayList<ArrayList<Integer>>();

        //Variavel auxiliar para armazenar as centrďż˝ides antigas
        float[][] oldCentroids;

        //indica se as centrďż˝ides foram modificadas
        boolean isCentroidModified = true;

        //serve para executar o algoritmo atďż˝ um nďż˝mero limitado de vazes
        int numberIterations = 0;

        //inicializar as centrďż˝ides com alguns pontos
        ...
        while (isCentroidModified && numberIterations < 30) {
            ...
            for (int dim = 0; dim < points[0].length; dim++) {
                int nearestCentroid = 0; //armazenarďż˝ a centrďż˝ide que a dimensďż˝o pertencerďż˝
                float distance1 = this.calculateDistance(points, dim, nearestCentroid);

                //para cada cluster existente, verificar qual centrďż˝ide a dimensďż˝o
                //estďż˝ mais prďż˝xima
            ...
            //Armazena os valores antigos das centrďż˝ides
            oldCentroids = this.centroids;

            //Atualiza as centrďż˝ides
//            this.updateCentroids(points, clusters);

            //Se nďż˝o houve modificaďż˝ďż˝o, para o looping
    ...
    public float calculateDistance(float[][] points, int dim, int nearestCentroid) {
