...
public abstract class AbstractDimensionalityReduction {
    ...
    public AbstractMatrix reduce(AbstractMatrix matrix, AbstractDissimilarity diss) {
        if (this.targetDimension >= matrix.getDimensions()) {
            throw new IllegalArgumentException("The target dimension should be smaller than the "
    ...
    protected abstract float[][] execute(AbstractMatrix matrix, AbstractDissimilarity diss);
