...
public class SimilarityMatrixCreation {
    ...
    public SimilarityMatrixModel execute(AbstractMatrix matrix, AbstractDissimilarity diss) {
        DistanceMatrix dmat = new DistanceMatrix(matrix, diss);
        return execute(dmat);
    }

    public SimilarityMatrixModel execute(DistanceMatrix dmat) {
        ...
        for (int i = 0; i < dmat.getElementCount(); i++) {
            ...
            if(!dmat.getLabels().isEmpty()) {
                label = dmat.getLabels().get(index[i]);
            }
            
            SimilarityMatrixInstance smi = new SimilarityMatrixInstance(label,
                    dmat.getIds().get(index[i]), i, dmat.getClassData()[index[i]]);
            model.addInstance(smi);
    ...
    class Order implements Comparable {
        ...
        public int compareTo(Object o) {
            if (!(o instanceof Order)) {
                throw new IllegalArgumentException("Given object is not comparable to this object.");
            }
            if (Math.abs(this.value - ((Order) o).value) == EPSILON) {
                // FIXME - maybe <= EPSILON?
                return 0;
            } else if (this.value - ((Order) o).value > EPSILON) {
                // FIXME - maybe just simply compare with this.value > ((Order) o).value?
                return 1;
            } else {
                return -1;
            }
