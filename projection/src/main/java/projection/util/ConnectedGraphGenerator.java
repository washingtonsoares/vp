    ...
    public void execute(Pair[][] neighborhood, AbstractMatrix matrix, AbstractDissimilarity diss) {
        ...
        for (int i = 0; i < neighborhood.length; i++) {
            for (int j = 0; j < neighborhood[i].length; j++) {
                ...
                if (!contain) {
                    ...
                    System.arraycopy(neighborhood[neighborhood[i][j].index], 0, newneighbors, 0, newneighbors.length - 1);
        ...
        while (visited.size() < neighborhood.length) {
            ...
            if (visited.size() > 0) {
                ...
                System.arraycopy(neighborhood[node], 0, newNeighbors1, 0, neighborhood[node].length);
                ...
                System.arraycopy(neighborhood[seed], 0, newNeighbors2, 0, neighborhood[seed].length);
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "Connecting components time: {0}s", (finish - start) / 1000.0f);
    }
