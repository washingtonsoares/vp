...
public class SammonMappingProjection implements Projection {
    ...
    public AbstractMatrix project(DistanceMatrix dmat) {
        ...
        AbstractMatrix projection = proj.project(dmat);

        if (projection != null) {
            SammonMapping sm = new SammonMapping();
            sm.setMF(magicfactor);

            float[][] aux_proj = projection.toMatrix();

            for (int i = 0; i < nriterations; i++) {
                sm.iteration(dmat, aux_proj);
            }
    ...
    /**
     * @param magicfactor the magicfactor to set
     */
    ...
    private float magicfactor = 0.3f;
}
