...
import projection.technique.fastmap.FastmapProjection3D;

/**
 *
 * @author Fernando Vieira Paulovich
 */
public class ForceSchemeProjection3D implements Projection {

    public ForceSchemeProjection3D() {
    ...
    public AbstractMatrix project(DistanceMatrix dmat) {
        ...
        if (projection == null) {            
            ...
            for (int i = 0; i < dmat.getElementCount(); i++) {
                ...
                point[0] = (float) (Math.random() * Math.sqrt(dmat.getMaxDistance()));
                point[1] = (float) (Math.random() * Math.sqrt(dmat.getMaxDistance()));
                point[2] = (float) (Math.random() * Math.sqrt(dmat.getMaxDistance()));  
        ...
        for (int i = 0; i < aux_proj.length; i++) {
            ...
            projection.getRow(i).setValue(2, aux_proj[i][2]);
        }
    ...
    private void iteration(DistanceMatrix dmat, float[][] aux_proj) {
        ...
        for (int ins1 = 0; ins1 < aux_proj.length; ins1++) {
            ...
            for (int ins2 = 0; ins2 < aux_proj.length; ins2++) {
                ...
                double z1z2 = (aux_proj[instance2][2] - aux_proj[instance][2]);
                ...
                aux_proj[instance2][2] += delta * (z1z2 / dr2);
            }
        }
    }
